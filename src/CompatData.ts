...
class CompatData {
    ...
    public constructor(sourceData: any = _data) {
        ...
        this.m_data = sourceData;
        this.m_supportedClientNames = Object.keys(this.m_data.browsers);
    }
    ...
    public getDisplayName(clientName: string): string {
        if (!(clientName in this.m_data.browsers)) {
            throw new RangeError("Invalid client name: " + clientName);
        }
        return this.m_data.browsers[clientName].name;
    }
        ...
        flags: ClientCompatCheckerFlags): readonly ClientCompatIssueList[]
    {
        ...
        _loadCompatDataFromSource(this.m_data.javascript.builtins, clientInfos, flags, clientIssuesLists);
        _loadCompatDataFromSource(this.m_data.api, clientInfos, flags, clientIssuesLists);

        return clientIssuesLists;
    }

    private _createClientMinVersionMap() {
        const browsers = this.m_data.browsers;
...
function _supportStatementVersionRemovedToObject(versionRemoved: any): Version {
    return (typeof(versionRemoved) === "string") ? Version.fromString(versionRemoved) : Version.infinite;
}
...
{
    const isWindowGlobal: boolean = (typeName === "Window" || typeName === "WindowOrWorkerGlobalScope");
