...
const clientInfo: ClientInfo = new ClientInfo("a", "a", Version.minVal, Version.maxVal);
...
function propIssues(list: ClientCompatIssueList, type: string, name: string): Issue[] {
    const arr: Issue[] = [];
    list.getIssuesForPropertyOrEvent(type, name, false, arr);
    return arr;
}

function eventIssues(list: ClientCompatIssueList, type: string, name: string): Issue[] {
    const arr: Issue[] = [];
...
beforeAll(() => {
    ...
    issueList.setIssuesForPropertyOrEvent("C", "p", false, [
        ...
        new Issue("C.p", clientInfo, IssueKind.NEEDS_ALT_NAME, "P", "Note for C.p (1)"),
...
test("ClientCompatIssueList.getIssuesForPropertyOrEvent", () => {
    ...
    expect(propIssues(issueList, "C", "p")).toMatchObject([
        ...
        {featureName: "C.p", kind: IssueKind.NEEDS_ALT_NAME, altOrPrefix: "P", note: "Note for C.p (1)"},
